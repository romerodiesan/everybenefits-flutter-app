...
type CountryRow = [iso2: string, dial: string, name: string, nameEs: string];
...
function priorityOf(iso2: string): number {
  const index = (PRIORITY_ISO2 as readonly string[]).indexOf(iso2);
  return index === -1 ? PRIORITY_ISO2.length + 1 : index;
}

export const PHONE_COUNTRIES: PhoneCountry[] = ROWS.map(
  ([iso2, dialCode, name, nameEs]) => ({
    iso2,
    dialCode,
    name,
    nameEs,
...
export function phoneCountryByIso2(
...
export function resolvePhoneCountry(input: {
  ...
  return PHONE_COUNTRIES.find((c) => c.dialCode === dial) ?? PHONE_COUNTRIES[0]!;
}
