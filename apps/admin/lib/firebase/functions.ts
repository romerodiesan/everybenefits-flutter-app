...
export async function listUsersForAdmin(filters?: {
  ...
  try {
    ...
    return {
      users: (data?.users ?? []).map(mapUserRow).filter((p) => p.uid),
...
export async function repairOrgTree() {
  return callCloudFunction<{ moved?: number }>("repairOrgTree", {});
}
