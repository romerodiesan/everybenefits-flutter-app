...
import { fetchCatalogInsights } from "@/lib/firebase/insights";
import {
  COURSE_LEVELS,
  type Course,
  type CourseLevel,
...
export function LibraryHome() {
  ...
  useEffect(() => {
    ...
    const maybeReady = () => {
      if (coursesReady && pathsReady) setListReady(true);
    };
    ...
    const stops = [
      ...
      watchAuthoredPaths(uid, onPaths, () => {
        setPaths([]);
        pathsReady = true;
        maybeReady();
      }),
  ...
  useEffect(() => {
    ...
    let cancelled = false;
    fetchCatalogInsights({ courseIds: courses.map((c) => c.id) })
      .then((result) => {
        if (!cancelled) setActiveStudents(result.kpis.activeStudents);
      })
      .catch(() => {
        if (!cancelled) {
          // Fallback: sum per-course in-progress counters (may double-count).
          setActiveStudents(
            courses.reduce((sum, c) => sum + (c.activeStudentCount ?? 0), 0),
          );
        }
      });
    return () => {
  ...
  }, [listReady, courseIdsKey, courses]);
  ...
  }, [tab, courses, paths, activeStudents]);
              ...
              >
                ...
                <div className="p-4">
                  ...
                  <p className="mt-3 text-[11px] text-muted">
                    {levelLabel(course.level)} · {course.lessonCount}{" "}
                    {t("libraryLessonsShort")} · {course.activeStudentCount}{" "}
                    {t("libraryInProgressShort")}
                  </p>
                </div>
              </Link>
            ))
