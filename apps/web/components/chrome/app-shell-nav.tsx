...
export function IconChat({ filled, ...props }: IconProps) {
  return (
    <svg viewBox="0 0 24 24" width="22" height="22" fill="none" {...props}>
      {filled ? (
        <path
...
export function IconAi({ filled, ...props }: IconProps) {
  return (
    <svg viewBox="0 0 24 24" width="22" height="22" fill="none" {...props}>
      <path
...
export function IconSchool({ filled, ...props }: IconProps) {
  return (
    <svg viewBox="0 0 24 24" width="22" height="22" fill="none" {...props}>
      {filled ? (
        <path
...
export function IconTools({ filled, ...props }: IconProps) {
  return (
    <svg viewBox="0 0 24 24" width="22" height="22" fill="none" {...props}>
      {filled ? (
        <path
...
export function IconPerson({ filled, ...props }: IconProps) {
  return (
    <svg viewBox="0 0 24 24" width="22" height="22" fill="none" {...props}>
      {filled ? (
        <>
...
export function IconBell({ filled, ...props }: IconProps) {
  return (
    <svg viewBox="0 0 24 24" width="22" height="22" fill="none" {...props}>
      <path
        fill={filled ? "currentColor" : "none"}
...
export type NavBadgeKind = "unread" | "forum" | "notifs";
...
export function AppShellSideNav({
  ...
  toolsActive: boolean;
  toolsOpen: boolean;
  setToolsOpen: (updater: boolean | ((v: boolean) => boolean)) => void;
  badgeFor: (kind: NavBadgeKind | undefined) => string | null;
  supportBusy: boolean;
  openSupport: () => void;
...
export function AppShellBottomNav({
  ...
  setMobileToolsOpen: (updater: boolean | ((v: boolean) => boolean)) => void;
  badgeFor: (kind: NavBadgeKind | undefined) => string | null;
}) {
  const t = useTranslations();
  const pathname = usePathname();
...
export function AppShellMobileTopBar({ role }: { role: UserRole }) {
  return (
    <div className="pointer-events-none absolute inset-x-0 top-0 z-30 flex items-start justify-between gap-2 px-3 pt-[max(0.75rem,env(safe-area-inset-top,0px)+0.35rem)] lg:hidden">
      <div className="pointer-events-auto min-w-0" data-tour="shell-apps">
        <AppSwitcher current="pulse" role={role} />
      </div>
...
export function AppShellSupportFab({
  supportBusy,
  openSupport,
}: {
  supportBusy: boolean;
