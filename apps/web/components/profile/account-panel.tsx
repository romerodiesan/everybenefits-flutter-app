...
import { startEmailChange } from "@/lib/firebase/profile-email";
import { Avatar, Button, Input, Label } from "@pulse/ui";
import {
  SettingsPanelShell,
  StatusBanner,
...
export function AccountPanel() {
  ...
  // After verifyBeforeUpdateEmail confirmation, Auth.email updates — sync Firestore.
  useEffect(() => {
    if (!profile || !user?.email) return;
    if (user.email === profile.email) return;
    void updateUserProfile(profile, { email: user.email }).then(() =>
      refreshProfile(),
    );
  }, [user?.email, profile, refreshProfile]);
  ...
  async function onSave(e: FormEvent) {
    ...
    try {
      ...
      if (code.includes("requires-recent-login")) {
        setError(t("emailChangeRequiresRecentLogin"));
      } else if (code.includes("email-already-in-use")) {
        setError(t("emailChangeAlreadyInUse"));
      } else if (code.includes("invalid-email")) {
        setError(t("emailChangeInvalid"));
      ...
      <form onSubmit={onSave} className="mt-5 space-y-4">
        ...
        <div className="max-w-sm">
          ...
          <Input
            ...
            autoComplete="email"
          />
          <p className="mt-1 text-xs text-muted">{t("emailChangeHint")}</p>
        </div>
        ...
        <div className="flex flex-wrap items-center gap-3">
          <Button
            type="submit"
            disabled={
              busy || (Boolean(verificationId) && smsCode.trim().length < 6)
            }
          >
            {emailChanged
              ? t("emailChangeVerify")
          ...
          {status === "email" && (
            <StatusBanner kind="info">{t("emailChangeSent")}</StatusBanner>
          )}
          {(status === "error" || error) && (
