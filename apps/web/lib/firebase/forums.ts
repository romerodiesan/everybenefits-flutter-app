...
/** Batched thread fetches (chunks of 10 via documentId `in`). */
export async function getThreads(threadIds: string[]): Promise<ForumThread[]> {
  ...
  for (let i = 0; i < unique.length; i += 10) {
    ...
    for (const d of snap.docs) {
      out.push(threadFrom(d.id, d.data() as Record<string, unknown>));
    }
  }
  return out;
}
...
export async function fetchThreadVotes(input: {
  ...
  // Point-get by deterministic doc ids (threadId) instead of loading all votes.
  const snaps = await Promise.all(
    input.threadIds.map((threadId) =>
      getDoc(doc(getFirebaseDb(), "users", input.uid, "forumVotes", threadId)),
    ),
  );
  for (const snap of snaps) {
    ...
    if (data.replyId == null) {
      out[snap.id] = Number(data.value ?? 0);
    }
  }
  return out;
}
...
export async function fetchReplyVotes(input: {
  ...
  for (let i = 0; i < snaps.length; i++) {
    const snap = snaps[i]!;
    const replyId = input.replyIds[i]!;
    if (!snap.exists()) continue;
