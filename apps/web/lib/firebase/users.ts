...
import { getFirebaseAuth, getFirebaseDb, getFirebaseStorage } from "@pulse/firebase-client";
...
function appearanceFrom(
  ...
  const locale =
    data.locale === "inherit" || data.locale === "en" || data.locale === "es"
      ? data.locale
      : undefined;
  if (!theme && !accent && !locale) return null;
  return {
    theme: theme ?? "light",
    accent: accent ?? "teal",
    locale: locale ?? "inherit",
  };
}
