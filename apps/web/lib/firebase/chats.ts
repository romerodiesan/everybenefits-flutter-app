...
import { getFirebaseAuth, getFirebaseRtdb } from "@pulse/firebase-client";
...
export function watchChatUnreadTotal(
  uid: string,
  onChange: (total: number) => void,
  onError?: (error: Error) => void,
): Unsubscribe {
  return onValue(
    ref(getFirebaseRtdb(), `userMeta/${uid}/chatUnreadTotal`),
    (snap) => {
...
export async function updateGroupChat(input: UpdateGroupChatInput) {
  const result = await callCloudFunction<{
    chatId?: string;
    memberCount?: number;
    truncated?: boolean;
    ...
    removeMemberIds: input.removeMemberIds ?? [],
    seedRoles: input.seedRoles ?? [],
    autoJoin: input.autoJoin === true,
    autoJoinRoles: input.autoJoinRoles,
...
export async function uploadGroupAvatar(
  ...
  // Base64 without spreading (avoids call-stack limits on large images).
  let binary = "";
  const chunk = 0x8000;
  ...
  const result = await callCloudFunction<{ photoUrl?: string }>(
    "uploadGroupAvatar",
    { chatId, contentType, data },
    { timeoutMs: 60_000 },
  );
  const url = String(result?.photoUrl ?? "").trim();
  if (!url) throw new Error("Avatar upload failed");
  return url;
}

/** Who may edit a group (not Support). Team is editable. */
export function canEditGroupChat(
  ...
  return chat.createdBy === viewer.uid;
}

/** Hard-delete for everyone — platform admins only; never Support. */
...
export async function deleteGroupChat(chatId: string) {
  return callCloudFunction<{ ok?: boolean; chatId?: string }>(
    "deleteGroupChat",
    { chatId },
    { timeoutMs: 60_000 },
  );
...
export async function markChatRead(chatId: string, uid: string) {
  const db = getFirebaseRtdb();
  // Only clear unread on chat + own inbox row. syncChatInbox applies
  // userMeta/chatUnreadTotal deltas so we don't double-decrement.
  await update(ref(db), {
...
export async function setReaction(input: {
  chatId: string;
  messageId: string;
  /** @deprecated Ignored — Auth uid is always used. */
  uid?: string;
  emoji: string | null;
}) {
  ...
  // Callable is best-effort sync only (avoids IAM/cold-start blocking UX).
  const path = `messages/${input.chatId}/${input.messageId}/reactions/${authUid}`;
  if (input.emoji) {
  ...
  void callCloudFunction("setChatReaction", {
    chatId: input.chatId,
