...
/** v4 keeps productTourVersion so the walkthrough does not flash on reload. */
const CACHE_KEY = "pulse_profile_v4";
...
type CachedProfile = {
  ...
  /** Last completed product-tour version — required to avoid walkthrough flash. */
...
function toCached(profile: UserProfile): CachedProfile {
  return {
    ...
    productTourVersion: Number(profile.productTourVersion ?? 0),
...
function revive(cached: CachedProfile): UserProfile {
  return {
    ...
    productTourVersion: Number(cached.productTourVersion ?? 0),
