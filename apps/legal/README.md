...
- Deploy (App Hosting): backend id `legal` with `apps/legal/apphosting.yaml`
  - Build/start come from `package.json` (`apphosting:build` / `start`). Do not set
    `scripts.buildCommand` in `apphosting.yaml` — the Node buildpack rewrites
    `package.json` and can emit `"dependencies": null`, which breaks pnpm 11.
