...
export type NotifyPayload = {
  ...
  actorId?: string;
  actorName?: string;
  /** Bypass preference gates (admin registration alerts). */
  force?: boolean;
};
...
export async function notifyUser(
  ...
  if (!payload.force && !channelAllowsInApp(prefs, channel)) return;
  ...
  const pushAllowed =
    (payload.force || channelAllowsPush(prefs, payload.type)) &&
    !payload.silent;
