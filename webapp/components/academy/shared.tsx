...
export function useLevelLabels() {
  ...
  return (level: CourseLevel) => {
    if (level === "intermediate") return t("academyLevelIntermediate");
    if (level === "advanced") return t("academyLevelAdvanced");
...
export function useLessonTypeLabels() {
  ...
  return (type: LessonType) => {
    if (type === "reading") return t("lessonTypeReading");
    if (type === "quiz") return t("lessonTypeQuiz");
    return t("lessonTypeVideo");
  };
}
...
export function StatusChip({ status }: { status: CourseStatus }) {
  ...
  const label =
    status === "published"
      ? t("courseStatusPublished")
      : status === "pending"
        ? t("courseStatusPending")
        : t("courseStatusDraft");
  const tone =
    status === "published"
      ? "bg-brand/14 text-brand"
      : status === "pending"
        ? "bg-[#FFB84D]/18 text-[#B45309] dark:text-[#FFB84D]"
